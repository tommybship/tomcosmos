...
- **M5 — Small bodies.** Ingest from JPL Small-Body Database, integrate in Mode A.

  - **M5a — single-asteroid round-trip (landed).** SBDB query → Keplerian elements → `tomcosmos.targeting.sbdb.state_at_epoch` Kepler-propagates to scenario epoch → ICRF barycentric `(r, v)` → Mode A integration via `tomcosmos.run`. Demo scenario `scenarios/apophis-30day.yaml`. Exit criteria met:
    - `astroquery.jplsbdb` query returns elements that convert to ICRF barycentric state to **<1 km** vs. JPL Horizons at the element epoch (float-precision noise; 69 m for Apophis).
    - Mode A 30-day propagation of Apophis from the SBDB element epoch agrees with Horizons to **72 m** position / 30 µm/s velocity.
  ...
  - **M5b — bulk asteroid ingestion + viewer scale (landed).**
    ...
    Also landed (slice 3):
    - **Viewer bulk-cohort path.** `viz.pyvista_viewer` now splits rendering: massive bodies (Sun, planets, moons) keep the per-body actor + trail + label treatment, but when a scenario declares more than 20 test particles the entire test-particle cohort renders as a single `pv.PolyData` point cloud. `set_sample` mutates `polydata.points` in-place — one numpy slice write per frame regardless of cohort size. Empirically: 1,000-body scenario hits **4,500 set_sample updates per second** off-screen (well above any monitor refresh rate). Sun + planets in mixed scenarios still render with full sphere actors. Threshold of 20 keeps small Lagrange / Hohmann demos on the existing path; only true population-scale runs flip to the bulk cohort.
